...
//C语言可以通过strstr函数找，用STL的string库可以通过find函数找，找到以后跳过一个T串的长度。例
//如：在abcacbcbcabscbc中找cbc，第一次找到了这个位置：abcacbcbcabscbc，找到这个下标后，会
//跳过整体cbc，也就是从这个位置继续找：abcacbcbcabscbc，否则如果你只跳一个字符，会导致
//cbcbc会被算成2次，而按照本题的题意，应该算一次
#include<iostream>
#include<string>
using namespace std;
int main()
